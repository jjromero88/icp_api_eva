...
{
    public class DocumentoEstructuraService : IDocumentoEstructuraService
    {
        private readonly HttpClient _httpClient;
        private readonly IConfiguration _configuration;

        public DocumentoEstructuraService(HttpClient httpClient, IConfiguration configuration)
        {
            _httpClient = httpClient;
            _configuration = configuration;
        }

        public async Task<List<DocumentoEstructuraResponse>> GetList(DocumentoEstructuraFilterRequest request, string? token)
        {
            try
            {
                string url = String.Format("{0}{1}",
                _configuration["Microservices:IcpAdmin:UrlBase"],
              _configuration["Microservices:IcpAdmin:Endpoints:DocumentoEstructura:GetList"]);
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    if (httpResponse.IsSuccessStatusCode)
                    {
                        ...
                        var result = JsonSerializer.Deserialize<IcpAdminResponse<List<DocumentoEstructuraResponse>>>(responseContent);

                        if (result == null)
                            throw new HttpRequestException($"Error en endpoint GteListDocumentoEstructura, no se obtuvieron resultados");

                        if (result.Error)
                            throw new ArgumentException($"{result.Message ?? "Ocurió un error en la petición"}");

                        return result.Payload ?? new List<DocumentoEstructuraResponse>();
                    }
                    else
                    {
