...
{
    public class ModalidadIntegridadService: IModalidadIntegridadService
    {
        private readonly HttpClient _httpClient;
        private readonly IConfiguration _configuration;

        public ModalidadIntegridadService(HttpClient httpClient, IConfiguration configuration)
        {
            _httpClient = httpClient;
            _configuration = configuration;
        }

        public async Task<List<ModalidadIntegridadResponse>> GetList(ModalidadIntegridadFilterRequest request, string? token)
        {
            try
            {
                string url = String.Format("{0}{1}",
                _configuration["Microservices:IcpAdmin:UrlBase"],
              _configuration["Microservices:IcpAdmin:Endpoints:ModalidadIntegridad:GetList"]);
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    if (httpResponse.IsSuccessStatusCode)
                    {
                        ...
                        var result = JsonSerializer.Deserialize<IcpAdminResponse<List<ModalidadIntegridadResponse>>>(responseContent);
                        ...
                        return result.Payload ?? new List<ModalidadIntegridadResponse>();
                    }
                    else
                    {
