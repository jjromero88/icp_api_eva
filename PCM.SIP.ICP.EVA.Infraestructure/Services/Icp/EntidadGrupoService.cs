...
{
    public class EntidadGrupoService : IEntidadGrupoService
    {
        private readonly HttpClient _httpClient;
        private readonly IConfiguration _configuration;

        public EntidadGrupoService(HttpClient httpClient, IConfiguration configuration)
        {
            _httpClient = httpClient;
            _configuration = configuration;
        }

        public async Task<List<EntidadGrupoResponse>> GetList(EntidadGrupoFilterRequest request, string? token)
        {
            try
            {
                string url = String.Format("{0}{1}",
                _configuration["Microservices:IcpAdmin:UrlBase"],
              _configuration["Microservices:IcpAdmin:Endpoints:EntidadGrupo:GetList"]);
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    if (httpResponse.IsSuccessStatusCode)
                    {
                        ...
                        var result = JsonSerializer.Deserialize<IcpAdminResponse<List<EntidadGrupoResponse>>>(responseContent);
                        ...
                        return result.Payload ?? new List<EntidadGrupoResponse>();
                    }
                    else
                    {
