...
namespace PCM.SIP.ICP.EVA.Infraestructure.Services
{
    public class EtapaService : IEtapaService
    {
        ...
        public EtapaService(HttpClient httpClient, IConfiguration configuration)
        {
            _httpClient = httpClient;
            _configuration = configuration;
        }

        public async Task<List<EtapaIcpResponse>> GetList(EtapaIcpFilterRequest request, string? token)
        {
            try
            {
                string url = String.Format("{0}{1}",
                _configuration["Microservices:IcpAdmin:UrlBase"],
              _configuration["Microservices:IcpAdmin:Endpoints:Etapa:GetList"]);
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    if (httpResponse.IsSuccessStatusCode)
                    {
                        ...
                        var result = JsonSerializer.Deserialize<IcpAdminResponse<List<EtapaIcpResponse>>>(responseContent);
                        ...
                        return result.Payload ?? new List<EtapaIcpResponse>();
                    }
                    else
                    {
