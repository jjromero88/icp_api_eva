...
{
    public class EntidadSectorService : IEntidadSectorService
    {
        private readonly HttpClient _httpClient;
        private readonly IConfiguration _configuration;

        public EntidadSectorService(HttpClient httpClient, IConfiguration configuration)
        {
            _httpClient = httpClient;
            _configuration = configuration;
        }

        public async Task<List<EntidadSectorResponse>> GetList(EntidadSectorFilterRequest request, string? token)
        {
            try
            {
                string url = String.Format("{0}{1}",
                _configuration["Microservices:IcpAdmin:UrlBase"],
              _configuration["Microservices:IcpAdmin:Endpoints:EntidadSector:GetList"]);
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    if (httpResponse.IsSuccessStatusCode)
                    {
                        ...
                        var result = JsonSerializer.Deserialize<IcpAdminResponse<List<EntidadSectorResponse>>>(responseContent);
                        ...
                        return result.Payload ?? new List<EntidadSectorResponse>();
                    }
                    else
                    {
