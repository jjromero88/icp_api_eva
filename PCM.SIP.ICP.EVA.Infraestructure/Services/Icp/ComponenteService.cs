...
namespace PCM.SIP.ICP.EVA.Infraestructure.Services
{
    public class ComponenteService : IComponenteService
    {
        ...
        public ComponenteService(HttpClient httpClient, IConfiguration configuration)
        {
            _httpClient = httpClient;
            _configuration = configuration;
        }

        public async Task<List<ComponenteIcpResponse>> GetList(ComponenteIcpFilterRequest request, string? token)
        {
            try
            {
                string url = String.Format("{0}{1}",
                _configuration["Microservices:IcpAdmin:UrlBase"],
              _configuration["Microservices:IcpAdmin:Endpoints:Componente:GetList"]);
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    if (httpResponse.IsSuccessStatusCode)
                    {
                        ...
                        var result = JsonSerializer.Deserialize<IcpAdminResponse<List<ComponenteIcpResponse>>>(responseContent);
                        ...
                        return result.Payload ?? new List<ComponenteIcpResponse>();
                    }
                    else
                    {
